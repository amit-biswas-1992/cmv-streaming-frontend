...
export const getServerSideProps = async (context) => {
  ...
  const data = await callApiGetWithoutToken(`/core/tag-wise-media-view-all/${id}`);
    ...
    <div className='font-body bg-slate-900 h-full'>
      <div className='pt-4 text-white mb-4'>
        ...
        {router.query.id === "NewRelease" ? (
          ...
          router.query.id === "MostPopular" ? (<h1 className='text-xl grid place-items-center'>Most Popular</h1>) :
            router.query.id === "Trending" ? (<h1 className='text-xl grid place-items-center'>Trending Now</h1>) : (
